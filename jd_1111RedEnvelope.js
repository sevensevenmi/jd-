...
function getCoupons(shareId = '') {
    return new Promise(resolve => {
        let opts = {
            url: `https://api.m.jd.com/api?functionId=getCoupons&appid=u&_=${Date.now()}&loginType=2&body={%22platform%22:4,%22unionActId%22:%2231134%22,%22actId%22:%22${$.actId}%22,%22d%22:%22yLiq9xF%22,%22unionShareId%22:%22${shareId}%22,%22type%22:1,%22eid%22:%22${$.eid}%22}&client=apple&clientVersion=8.3.6&h5st=undefined`,
            headers: {
...
function shareUnionCoupon() {
    return new Promise(resolve => {
        let opts = {
            url: `https://api.m.jd.com/api?functionId=shareUnionCoupon&appid=u&_=${Date.now()}&loginType=2&body={%22unionActId%22:%2231134%22,%22actId%22:%22${$.actId}%22,%22platform%22:4,%22unionShareId%22:%22${$.shareCode}%22,%22d%22:%22yLiq9xF%22,%22supportPic%22:2,%22supportLuckyCode%22:0,%22eid%22:%22${$.eid}%22}&client=apple&clientVersion=8.3.6`,
            headers: {
                "Accept-Language": "zh-cn",
                "Accept-Encoding": "gzip, deflate, br",
                'Cookie': `${cookie} ${newCookie}`,
                ...
                if (err) {
                    ...
                    if (typeof res == 'object') {
                        if (res.code == 0 && res.data && res.data.shareUrl) {
                            $.shareCode = res.data.shareUrl.match(/yLiq9xF\?s=([^&]+)/) && res.data.shareUrl.match(/yLiq9xF\?s=([^&]+)/)[1] || ''
...
function getUrl() {
    return new Promise(resolve => {
        const options = {
            url: `https://u.jd.com/yLiq9xF?s=${$.shareCode}`,
            followRedirect: false,
            headers: {
                'Cookie': `${cookie} ${newCookie}`,
                "User-Agent": $.UA
