...
function reward_query() {
  return new Promise(resolve => {
    $.get(taskGetUrl("spring_reward_query", {
      "inviter": ["fVJCSjWS4sF0D1OF8znp1A"][Math.floor((Math.random() * 1))],
      linkId
    }), async (err, resp, data) => {
      try {
        if (err) {
...
async function redPacket() {
  return new Promise(resolve => {
    $.get(taskGetUrl("spring_reward_receive",{"inviter":["fVJCSjWS4sF0D1OF8znp1A"][Math.floor((Math.random() * 1))], linkId}),
        async (err, resp, data) => {
          try {
            if (err) {
              console.log(`${JSON.stringify(err)}`)
