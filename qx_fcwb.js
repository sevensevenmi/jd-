						...
						if($.index===1){
							if(QQ00OQ.success==true){
								...
								if(QQ00OQ.data&&QQ00OQ.data.inviteCode&&(inviteCodes.length===0)){
									inviteCodes.push({'user':$.UserName,'fcwbinviteCode':QQ00OQ.data.inviteCode,'fcwbinviter':QQ00OQ.data.markedPin});
								}
							}
...
function help(){
	return new Promise(async QO0O0O=>{
		...
		let O0O000={'appid':'activities_platform','functionId':'happyDigHelp','clientVersion':'1.0.0','client':'H5','t':OOQQ0O,'body':JSON.parse(OOQ000)};
		let O0O0Q0=await getH5st('8dd95',O0O000);
		let O0OQQO={'url':'https://api.m.jd.com/?functionId=happyDigHelp&body='+OOQ000+'&t='+OOQQ0O+'&appid=activities_platform&client=H5&clientVersion=1.0.0&h5st='+encodeURIComponent(O0O0Q0),'headers':{'Cookie':cookie,'Origin':'https://api.m.jd.com','User-Agent':UA}};
...
function TotalBean(){
	return new Promise(async O0O0QQ=>{
		const QOQQOQ={'url':'https://wq.jd.com/user/info/QueryJDUserInfo?sceneval=2','headers':{
...
function taskurl(OQQOOO,Q00O0Q){
	return{'url':JD_API_HOST+'/?functionId='+OQQOOO+'&body='+escape(JSON.stringify(Q00O0Q))+'&t=1635561607124&appid=activities_platform&client=H5&clientVersion=1.2.0','headers':{'Cookie':cookie,'Origin':'https://bnzf.jd.com','User-Agent':' jdltapp;iPhone;3.7.6;'}};
}
function getH5st(OOO0OO,Q00OQQ){
	return new Promise(async Q00OQO=>{
		let OQQ00O={'url':'http://api.kingran.cf/h5st','body':'businessId='+OOO0OO+'&req='+encodeURIComponent(JSON.stringify(Q00OQQ)),'headers':{'User-Agent':'Mozilla/5.0 (iPhone; CPU iPhone OS 13_2_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/13.0.3 Mobile/15E148 Safari/604.1 Edg/87.0.4280.88'},'timeout':(30*1000)};
